...
import {
  ...
  BarChart3, IndianRupee, Filter, X, BoxIcon, RefreshCw, Plus, ShoppingCart
} from "lucide-react";
...
const BoutiqueInventory = ({ products }: BoutiqueInventoryProps) => {
  const { orders, sales, getProductPrice, placeOrder } = useBoutique();
  ...
  const inventory: InventoryItem[] = useMemo(() => {
    ...
    orders.forEach(order => {
      ...
      const existing = inventoryMap.get(order.productId);
      if (existing) {
        ...
        inventoryMap.set(order.productId, {
          ...
          totalSold: 0,
          currentStock: 0,
          totalInvestment: order.buyingPrice * order.quantity,
          avgBuyingPrice: order.buyingPrice,
    ...
    sales.forEach(sale => {
      ...
      if (item) {
        item.totalSold += sale.quantity;
      }
    });

    inventoryMap.forEach(item => {
      const deliveredQty = item.orders
  ...
  const productsNotInInventory = products.filter(
    p => !inventory.some(i => i.productId === p.id) && p.stock > 0
  );
  ...
  const handleAddProduct = () => {
    ...
    const sellingPrice = addSellingPrice ? parseFloat(addSellingPrice) : product.mrp;
    ...
    toast({
      ...
      description: `${addQty} x ${product.name} ordered from Zarkha.`,
    });
  ...
  const handleQuickReorderAll = () => {
    ...
    if (lowItems.length === 0) {
      toast({ title: "All stocked!", description: "No low or out-of-stock items." });
      return;
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'out_of_stock':
        return <Badge className="bg-red-100 text-red-700 hover:bg-red-100"><TrendingDown className="h-3 w-3 mr-1" />Out of Stock</Badge>;
      default:
        return null;
    }
  };
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-2 sm:gap-3">
        ...
        <div className="flex gap-2 sm:ml-auto w-full sm:w-auto">
          {(lowStockCount > 0 || outOfStockCount > 0) && (
            <Button variant="brand" size="sm" onClick={handleQuickReorderAll} className="flex-1 sm:flex-none text-xs sm:text-sm">
              <RefreshCw className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1" /> Reorder Low
            </Button>
          )}
          <Button variant="outline" size="sm" onClick={() => setShowAddDialog(true)} className="flex-1 sm:flex-none text-xs sm:text-sm">
        ...
        <CardContent>
          {filteredInventory.length === 0 ? (
            <div className="text-center py-12 border-2 border-dashed rounded-lg">
              ...
              {inventory.length === 0 && (
                <Button variant="brand" size="sm" className="mt-4" onClick={() => setShowAddDialog(true)}>
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {(item.status === 'low_stock' || item.status === 'out_of_stock') && (
                              <Button variant="brand" size="sm" onClick={() => handleReorder(item)}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          <div className="bg-muted/50 rounded-lg p-2">
                            ...
                            <p className="text-sm font-semibold">{item.totalPurchased} pcs</p>
                            <p className="text-xs text-muted-foreground">@ ₹{Math.round(item.avgBuyingPrice)}/pc</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2">
                            <p className="text-xs text-muted-foreground">Sold</p>
                            <p className="text-sm font-semibold text-green-600">{item.totalSold} pcs</p>
                            <p className="text-xs text-muted-foreground">@ ₹{item.sellingPrice}/pc</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2">
                            <p className="text-xs text-muted-foreground">Investment</p>
                            <p className="text-sm font-semibold text-brand-orange">₹{item.totalInvestment.toLocaleString()}</p>
                          </div>
                          <div className="bg-muted/50 rounded-lg p-2">
                            <p className="text-xs text-muted-foreground">Potential Profit</p>
                            <p className="text-sm font-semibold text-green-600">₹{Math.max(0, potentialProfit).toLocaleString()}</p>
                          </div>
                        </div>
                        ...
                        {pendingQty > 0 && (
                          <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-700">
                            ...
                            <span>{pendingQty} units in transit ({pendingOrders.length} order{pendingOrders.length > 1 ? 's' : ''})</span>
                          </div>
                        )}

                        {/* Purchase History */}
                        <div className="space-y-1">
                          <p className="text-xs font-medium text-muted-foreground">Purchase History</p>
                          <div className="flex flex-wrap gap-1.5">
                            {item.orders.map(order => (
                              <div key={order.id} className="flex items-center gap-1.5 px-2 py-1 bg-muted rounded text-xs">
                                ...
                                <span>{order.id}</span>
                                <span className="text-muted-foreground">• {order.quantity}pcs</span>
          ...
          {reorderItem && (
            <div className="space-y-4">
              <div className="flex gap-3 p-3 bg-muted/50 rounded-lg">
                <img src={reorderItem.productImage} alt={reorderItem.productName} className="w-16 h-16 object-cover rounded" />
                <div>
                  <p className="font-semibold text-sm">{reorderItem.productName}</p>
                  <p className="text-xs text-muted-foreground">{reorderItem.category}</p>
                  <div className="flex items-center gap-2 mt-1">
                    {getStatusBadge(reorderItem.status)}
                    <span className="text-xs text-muted-foreground">Current: {reorderItem.currentStock} pcs</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Unit Price (Zarkha)</span>
                  <span className="font-medium">₹{reorderItem.avgBuyingPrice.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between text-base font-bold">
                  ...
                  <span className="text-brand-orange">₹{(reorderItem.avgBuyingPrice * reorderQty).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-xs text-green-600">
                  <span>Expected Profit (per unit)</span>
                  <span>₹{(reorderItem.sellingPrice - reorderItem.avgBuyingPrice).toLocaleString()}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
                ...
                <SelectContent>
                  {products.filter(p => p.stock > 0).map(p => (
                    <SelectItem key={p.id} value={p.id}>
                      <div className="flex items-center gap-2">
                        <span>{p.name}</span>
                        <span className="text-muted-foreground text-xs">₹{p.adminPrice}</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              {productsNotInInventory.length > 0 && (
                <p className="text-xs text-muted-foreground">
                  {productsNotInInventory.length} product{productsNotInInventory.length > 1 ? 's' : ''} not yet in your inventory
                </p>
              )}
            </div>
              ...
              return (
                <div className="flex gap-3 p-3 bg-muted/50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-brand-orange mt-1">Zarkha Price: ₹{product.adminPrice}</p>
                  </div>
                </div>
              );
            })()}
            ...
            {addProductId && (() => {
              const product = products.find(p => p.id === addProductId);
              if (!product) return null;
              const sp = addSellingPrice ? parseFloat(addSellingPrice) : product.mrp;
              return (
                <>
                  <Separator />
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-brand-orange">₹{(product.adminPrice * addQty).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-green-600">
                      <span>Profit per unit</span>
                      <span className="font-medium">₹{(sp - product.adminPrice).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-green-600">
                      <span>Total Potential Profit</span>
                      <span className="font-bold">₹{((sp - product.adminPrice) * addQty).toLocaleString()}</span>
                    </div>
                  </div>
                </>
              );
          ...
          <DialogFooter>
            ...
            <Button variant="brand" onClick={handleAddProduct} disabled={!addProductId}>

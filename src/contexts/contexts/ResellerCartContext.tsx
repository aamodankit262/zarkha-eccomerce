...
interface ResellerCartContextType {
  ...
  addItem: (item: Omit<ResellerCartItem, 'quantity' | 'sellingPrice'>) => void;
...
export const ResellerCartProvider = ({ children }: { children: ReactNode }) => {
  const [items, setItems] = useState<ResellerCartItem[]>([]);
  const [customerInfo, setCustomerInfo] = useState<CustomerInfo>({ name: '', phone: '', email: '' });
  const [shippingAddress, setShippingAddress] = useState<Address>({ name: '', phone: '', address: '', city: '', state: '', pincode: '' });
  const [billingAddress, setBillingAddress] = useState<Address>({ name: '', phone: '', address: '', city: '', state: '', pincode: '' });
  const [sameAsShipping, setSameAsShipping] = useState(true);

  const addItem = (item: Omit<ResellerCartItem, 'quantity' | 'sellingPrice'>) => {
  ...
  const updateSellingPrice = (id: string, price: number) => {
    setItems(prev => prev.map(item => item.id === id ? { ...item, sellingPrice: price } : item));
  };
  ...
  return (
    <ResellerCartContext.Provider value={{
      items, customerInfo, shippingAddress, billingAddress, sameAsShipping,
      addItem, updateQuantity, updateSellingPrice, removeItem, clearCart,
